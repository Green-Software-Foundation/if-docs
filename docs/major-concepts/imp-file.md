...
The IMP is a [yaml](https://circleci.com/blog/what-is-yaml-a-beginner-s-guide/) file with a particular structure.
...
This is possible because _all the configuration and data required to run Impact Framework is contained in the IMP file_.

Anyone can download Impact Framework and execute an IMP file to verify the results.
...
The initialize section is where you define which plugins will be used in your IMP file and provide the configuration for them. Below is sample for initialization:
...
Impact Framework uses the `initialize` section to instantiate each plugin. A plugin cannot be invoked elsewhere in the IMP file unless it is included in this section.
...
This section is auto generated by IF at runtime. You don't have to include this section in your IMP. The `execution` node contains all the necessary information to rebuild the environment, which can support debugging or verifying output files.
...
The plugins in the `observe` part of the pipeline generate `input` data. The IMP file should not have `input` data when the `observe` phase is executed. Plugins in this phase _only_ generate input data, they can never generate output data. If you run the observe phase on its own (by running `if-run --observe`) then your IMP will be returned populated with input data according to the plugins you included in your `observe` pipeline.

## Regrouping an IMP file

The second phase of IMP execution is `regroup`. This reorganizes existing `input` data into a new structure using keys provided in the `regroup` config in the IMP. For example, an IMP with the following `tree`:
...
## Computing an IMP file

Impact Framework computes IMP files. For each component in the tree, the `inputs` array is passed to each plugin in the `compute` pipeline in sequence.

In order for the `compute` phase to execute correctly, the IMP needs to have `input` data available.
...
This implies a sequence of plugins where the inputs for some plugins must either be present in the original IMP file or be outputs of the preceding plugins in the pipeline.
...
It is possible to run each phase of the execution individually, or together. You can choose to _only_ run the `observe`, `regroup` or `compute` phases of the IMP execution. This saves you from having to re-execute entire IMPs every time you want to tweak something, making it a greener way to use IF.
...
```
if-run -m <imp> --regroup --compute
```

## Outputs

When Impact Framework computes an IMP file, it appends new data to the IMP file and the final result is an enriched IMP that includes all the configuration and contextual data, the input data, and the results of executing each plugin. This means the output file is completely auditable - the original IMP file can be recovered simply by deleting the `outputs` and `execution` sections of the output file.
